#!/usr/bin/env python3
"""Aion Comprehensive Test Suite — validates all project components."""
...
class TestFileIntegrity(unittest.TestCase):
    ...
    EXPECTED_JSON = [
        "config/aion-config.json",
        "core/input-engine/input-mapping.json",
        "android/key-mapper/keymaps.json",
        "deploy/ota/manifest.json",
    ]
    EXPECTED_SERVICE = [
        "core/security/aion-security.service",
        "core/services/aion-init.service",
        "core/input-engine/aion-input.service",
        "android/key-mapper/aion-key-mapper.service",
        "deploy/ota/aion-ota.service",
        "performance/throttler/aion-throttler.service",
        "ui/oobe/aion-oobe.service",
    ]
    EXPECTED_TIMER = ["deploy/ota/aion-ota.timer"]
    EXPECTED_YAML = [
        "deploy/github/release-pipeline.yml",
        "deploy/github/pages-deploy.yml",
    ]
    EXPECTED_MISC = [
        "android/apk-installer/aion-apk-handler.desktop",
        "core/security/selinux-policy.te",
        "core/security/selinux-policy.fc",
        "ui/plasma-config/aion-plasma.conf",
...
class TestPythonImports(unittest.TestCase):
    ...
    def test_ota_silent_mode_cli(self):
        src = _read("deploy/ota/ota-updater.py")
        self.assertIn("--silent", src, "ota-updater missing --silent flag")
        self.assertIn("--defer-if-busy", src, "ota-updater missing --defer-if-busy flag")
        self.assertIn("def system_busy", src, "ota-updater missing system_busy guard")
        self.assertIn("/proc/loadavg", src, "ota-updater missing load guard")
        self.assertIn("/proc/uptime", src, "ota-updater missing uptime guard")
        self.assertIn("apply_update(silent=", src, "ota-updater must pass silent flag")

    def test_ota_silent_service_and_timer(self):
        svc = _read("deploy/ota/aion-ota-silent.service")
        timer = _read("deploy/ota/aion-ota-silent.timer")
        self.assertIn("--silent", svc, "silent service missing --silent")
        self.assertIn("--defer-if-busy", svc, "silent service missing --defer-if-busy")
        self.assertIn("Nice=15", svc, "silent service should run at lowest priority")
        self.assertIn("IOSchedulingClass=idle", svc, "silent service should be idle IO class")
        self.assertIn("OnUnitActiveSec=1d", timer, "silent timer should run daily at most")

    def test_ota_silent_service_lists_expected_files(self):
        p = ROOT / "deploy/ota"
        self.assertTrue((p / "aion-ota-silent.service").exists())
        self.assertTrue((p / "aion-ota-silent.timer").exists())

    def test_footprint_optimize_script(self):
        src = _read("build/scripts/06-footprint-optimize.sh")
        for needle in [
            "paccache",
            "__pycache__",
            "journald.conf.d",
            ...
            self.assertIn(needle, src, f"06-footprint-optimize.sh missing: {needle}")

    def test_build_calls_footprint_script(self):
...
class TestConfiguration(unittest.TestCase):
    ...
    def test_aion_config_structure(self):
        cfg = self._load_json("config/aion-config.json")
        for key in ["system", "security", "input", "performance", "display", "android", "ota", "chrome"]:
            self.assertIn(key, cfg, f"aion-config.json missing top-level key: {key}")

    def test_aion_config_system(self):
        sys_cfg = self._load_json("config/aion-config.json")["system"]
        self.assertEqual(sys_cfg["name"], "Aion")
        self.assertEqual(sys_cfg["version"], "1.0.0")
        self.assertIn(sys_cfg["selinux_mode"], ["enforcing", "permissive", "disabled"])

    def test_aion_config_security(self):
        sec = self._load_json("config/aion-config.json")["security"]
    ...
    def test_aion_config_performance(self):
        perf = self._load_json("config/aion-config.json")["performance"]
    ...
    def test_aion_config_display(self):
        disp = self._load_json("config/aion-config.json")["display"]
    ...
    def test_aion_config_android(self):
        andr = self._load_json("config/aion-config.json")["android"]
        self.assertTrue(andr["waydroid_enabled"])
        self.assertIn(andr["waydroid_image"], ["VANILLA", "GAPPS"])

    def test_aion_config_ota(self):
        ota = self._load_json("config/aion-config.json")["ota"]
        self.assertIn("manifest_url", ota)
        self.assertGreater(ota["check_interval_hours"], 0)

    def test_aion_config_chrome(self):
        chrome = self._load_json("config/aion-config.json")["chrome"]
    ...
    def test_plasma_conf_colors(self):
        src = _read("ui/plasma-config/aion-plasma.conf")
...
class TestSecurity(unittest.TestCase):
    ...
    def test_selinux_policy_defines_types(self):
        ...
        self.assertIn("aion_game_t", src, "Missing type aion_game_t")
        self.assertIn("aion_bypass_t", src, "Missing type aion_bypass_t")
    ...
    def test_selinux_fc_has_labels(self):
        ...
        self.assertIn("aion", src, "selinux-policy.fc should reference aion types")

    def test_security_service_hardening(self):
        src = _read("core/security/aion-security.service")
    ...
    def test_input_service_security(self):
        src = _read("core/input-engine/aion-input.service")
...
class TestPerformance(unittest.TestCase):
    def test_zram_enabled(self):
        cfg = json.loads(_read("config/aion-config.json"))
    ...
    def test_throttler_slices(self):
        src = _read("performance/throttler/resource-throttler.py")
        self.assertIn("aion-gaming.slice", src)
        self.assertIn("aion-install.slice", src)

    def test_throttler_uses_proportional_free_ram(self):
        src = _read("performance/throttler/resource-throttler.py")
        self.assertIn("resolve_target_free_mb", src)
        self.assertIn("AION_PERFORMANCE_PROFILE", src)
        self.assertIn("performance-profile.json", src)

    def test_performance_engine_exists(self):
        src = _read("performance/engine/aion-performance-engine.py")
        self.assertIn("class PerformanceRating", src)
        self.assertIn("fps_target", src)
        self.assertIn("model-agnostic", src)

    def test_performance_engine_service(self):
        src = _read("performance/engine/aion-performance-engine.service")
        self.assertIn("ExecStart=/usr/bin/python3", src)
        self.assertIn("aion-performance-engine.py", src)
        self.assertIn("After=", src)
        self.assertIn("WantedBy=multi-user.target", src)

    def test_performance_engine_no_fixed_limits(self):
        src = _read("performance/engine/aion-performance-engine.py")
        self.assertIn("_saturate", src, "saturation must be closed-form")

    def test_hub_server_exists(self):
    ...
    def test_hub_service(self):
        src = _read("hub/aion-hub.service")
        self.assertIn("ExecStart=/usr/bin/python3", src)
        self.assertIn("aion-hub-server.py", src)
        self.assertIn("WantedBy=multi-user.target", src)

    def test_hub_password_module(self):
        src = _read("hub/aion-hub-pass.py")
        self.assertIn("pbkdf2_hmac", src)
        self.assertIn("compare_digest", src)
        self.assertIn("token_bytes", src)
        self.assertNotIn("hashlib.md5", src, "md5 is forbidden for secrets")


        self.assertNotIn("RTX", src, "no fixed GPU model names")
        self.assertNotIn("GeForce", src, "no fixed GPU model names")
        self.assertNotIn("Radeon", src, "no fixed GPU model names")

    def test_ota_timer_interval(self):
        src = _read("deploy/ota/aion-ota.timer")
...
class TestAndroid(unittest.TestCase):
    ...
    def test_key_mapper_requires_waydroid(self):
        src = _read("android/key-mapper/aion-key-mapper.service")
    ...
    def test_apk_handler_desktop_mime(self):
        src = _read("android/apk-installer/aion-apk-handler.desktop")
...
class TestUI(unittest.TestCase):
    ...
    def test_display_config_colors_match_plasma(self):
        cfg = json.loads(_read("config/aion-config.json"))
        plasma = _read("ui/plasma-config/aion-plasma.conf")
